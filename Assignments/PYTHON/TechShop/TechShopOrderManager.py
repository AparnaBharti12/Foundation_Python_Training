...
class OrderManager:
    ...
    def add_order(self, order: Orders):
        try:
            ...
            sql = ('INSERT INTO Orders(OrderID, CustomerID, OrderDate, TotalAmount, OrderStatus) VALUES (%s, %s, %s, '
                   '%s, %s, %s)')
            para = (order.get_order_id(), order.get_customer().get_customer_id(), order.get_order_date(),
    ...
    def remove_canceled_orders(self):
        try:
            ...
            sql1 = '''DELETE FROM OrderDetails WHERE OrderID IN
                    (SELECT OrderID FROM Orders WHERE status IN ('Cancelled', 'cancelled')'''
            sql2 = '''DELETE FROM Orders WHERE status IN ('Cancelled', 'cancelled')'''
